...
parser.add_argument(
    "--prompt",
    choices=["zero_shot", "rag", "compress"],
    default="zero_shot",
...
prompt_rag_regression_template = PromptTemplate(
    input_variables=["movie_name", "candidate", "movie_info"],
    template=prompt_rag_regression,
)

prompt_compress_template = PromptTemplate(
    input_variables=["movie_info"], template=prompt_compress
)
...
if args.prompt == "zero_shot":
    ...
    for index, row in tqdm(
        test_data.iterrows(), total=len(test_data), desc="Processing"
    ):
        uid = row["UserID"]
        ...
        user_ratings = train_data[train_data["UserID"] == uid].sample(
            n=5, random_state=42
        )
        ...
        total_cost = total_cost + get_llm_chat_cost(
            prompt_template.invoke(
                {
                    "history_ratings": history_movie_details_all,
                    "candidate": movie_details,
                }
            ).text,
            "input",
        )
    ...
    for index, row in tqdm(
        ...
        loader = DirectoryLoader(
            "./resources/datasets/wikidocs/",
            glob="{}*.txt".format(replace_punctuation_and_spaces(movie_title)),
